...
def main():
    ...
    if not params["skip_video"]:
        for delay_ms in [100, 1000]:
            frames_list = []
            env_params = MotionPlanningEnvParams()
            logger.info(f"Making video with delay {delay_ms}ms")
            df, frames = delay(
                env_params=env_params,
                policy=policy,
                max_steps=params["max_steps"],
                num_episodes=10,
                comm_interval=delay_ms / 1000,
                render=True,
            )
            frames_list.append(frames)
            logger.info("Saving video")
            frames = np.concatenate(frames_list)
            iio.imwrite(path / f"{name}_delay_{delay_ms}ms.mp4", frames, fps=40)

...
parser.set_defaults(
    greedy=False, expert=False, explore=False, render=False, n=100, full=False
)
...
env = gym.wrappers.FilterObservation(env, dict_keys=env.env.keys)
        ...
        if args.expert:
            try:
                action = env.env.env.controller(
                    random=False, greedy=False, reset_solution=False
                )
...
print(
    "Reward over {} episodes: mean = {:.1f}, std = {:.1f}".format(
        N, mean_reward, std_reward
    )
)

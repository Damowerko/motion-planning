...
class MultiAgentStateWithDelay(object):
    def __init__(self, device, n_states, n_agents, k, env_state, prev_state=None):
        """
        ...
        self.curr_gso[0, 0, :, :] = (
            torch.eye(n_agents).view((1, 1, n_agents, n_agents)).to(device)
        )  # I
        for k_ind in range(1, k):
            self.curr_gso[0, k_ind, :, :] = torch.matmul(
                self.network, self.curr_gso[0, k_ind - 1, :, :]
            )
        ...
        if prev_state is not None and k > 1:
            self.delay_gso[0, 1:k, :, :] = torch.matmul(
                self.network, prev_state.delay_gso[0, 0 : k - 1, :, :]
            )

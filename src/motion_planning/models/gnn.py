...
class GraphFilter(nn.Module):
    def __init__(
        ...
        out_channels: int,
        n_taps: int,
        aggr: str | gnn.Aggregation | typing.List[str | gnn.Aggregation] = "sum",
    ):
        """
        ...
        aggr_size = 1 if type(aggr) == str or type(aggr) == gnn.Aggregation else len(aggr)
        self.node_filter = gnn.Linear(in_channels, out_channels)
        self.pipe = nn.ModuleList(
            [gnn.Linear(aggr_size * in_channels, in_channels) for _ in range(n_taps)]
        )
        self.taps = nn.ModuleList(
            [gnn.Linear(in_channels, out_channels) for _ in range(n_taps)]
        )

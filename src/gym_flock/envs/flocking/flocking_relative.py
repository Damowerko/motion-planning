...
class FlockingRelativeEnv(gym.Env):
    def __init__(self):
        ...
        self.mean_pooling = (
            True  # normalize the adjacency matrix by the number of neighbors or not
        )
    ...
    def step(self, u):
        ...
        self.x[:, 0] = (
            self.x[:, 0]
            + self.x[:, 2] * self.dt
            + self.u[:, 0] * self.dt * self.dt * 0.5
        )
        # y position
        self.x[:, 1] = (
            self.x[:, 1]
            + self.x[:, 3] * self.dt
            + self.u[:, 1] * self.dt * self.dt * 0.5
        )
        # x velocity
    ...
    def compute_helpers(self):
        ...
        self.r2 = np.multiply(self.diff[:, :, 0], self.diff[:, :, 0]) + np.multiply(
        ...
        )  # correct - checked this
        n_neighbors[n_neighbors == 0] = 1
        self.adj_mat_mean = self.adj_mat / n_neighbors

        self.x_features = np.dstack(
            (
                self.diff[:, :, 2],
                np.divide(self.diff[:, :, 0], np.multiply(self.r2, self.r2)),
                np.divide(self.diff[:, :, 0], self.r2),
                self.diff[:, :, 3],
                np.divide(self.diff[:, :, 1], np.multiply(self.r2, self.r2)),
                np.divide(self.diff[:, :, 1], self.r2),
            )
        )

        self.state_values = np.sum(
            self.x_features * self.adj_mat.reshape(self.n_agents, self.n_agents, 1),
            axis=1,
        )
    ...
    def get_stats(self):
        ...
        stats["vel_diffs"] = np.sqrt(
            np.sum(
                np.power(self.x[:, 2:4] - np.mean(self.x[:, 2:4], axis=0), 2), axis=1
            )
        )
    ...
    def reset(self):
        ...
        while degree < 2 or min_dist < min_dist_thresh:
            ...
            x[:, 2] = (
                np.random.uniform(
                    low=-self.v_max, high=self.v_max, size=(self.n_agents,)
                )
                + bias[0]
            )
            x[:, 3] = (
            ...
            a_net = np.sum(
                np.square(
                    np.transpose(x_loc, (0, 2, 1)) - np.transpose(x_loc, (2, 0, 1))
                ),
                axis=2,
            )
    ...
    def controller(self, centralized=None):
        ...
        potentials = np.dstack(
            (
                self.diff,
                self.potential_grad(self.diff[:, :, 0], self.r2),
                self.potential_grad(self.diff[:, :, 1], self.r2),
            )
        )
        if not centralized:
            potentials = potentials * self.adj_mat.reshape(
        ...
        controls = np.hstack(
            (
                (-p_sum[:, 4] - p_sum[:, 2]).reshape((-1, 1)),
                (-p_sum[:, 3] - p_sum[:, 5]).reshape(-1, 1),
            )
        )
    ...
    def potential_grad(self, pos_diff, r2):
        ...
        """
        grad = -2.0 * np.divide(pos_diff, np.multiply(r2, r2)) + 2 * np.divide(
            pos_diff, r2
        )

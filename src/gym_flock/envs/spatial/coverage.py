...
from gym_flock.envs.spatial.utils import (
    _get_graph_edges,
    _get_k_edges,
    _get_pos_diff,
    _nodes_within_radius,
)
...
class CoverageEnv(gym.Env):
    def __init__(
        self,
        n_robots=N_ROBOTS,
        frac_active_targets=FRAC_ACTIVE,
        xmax=XMAX,
        ymax=YMAX,
        starts=start_regions,
        unvisiteds=unvisited_regions,
        init_graph=True,
        ...
        # square lattice
        self.lattice_vectors = [np.array([-DELTA, 0.0]), np.array([0.0, -DELTA])]
    ...
    def step(self, action):
        ...
        if action is not None:
            ...
            for i in range(self.n_robots):
                if next_locs[i] == -1:
                    next_loc = self.mov_edges[1][np.where(self.mov_edges[0] == i)][
                        action[i]
                    ]
    ...
    def get_action_edges(self):
        ...
        for i in range(self.n_robots):
            next_nodes = self.motion_edges[1][
                np.where(self.motion_edges[0] == curr_nodes[i])
            ]
            n_next_nodes = np.shape(next_nodes)[0]

            # pad edges for each robot with loops to current node
            if n_next_nodes < N_ACTIONS:
                next_nodes = np.append(
                    next_nodes, [curr_nodes[i]] * (N_ACTIONS - n_next_nodes)
                )
    ...
    def _get_obs_reward(self):
        ...
        if REVISIT_NODES:
            self.visited = np.where(
                self.landmark_flag
                * np.random.binomial(1, 0.005, size=np.shape(self.visited))
                > 0.0,
                0.0,
                self.visited,
            )
        ...
        if PAD_ACTIONS:
            ...
            action_edges, action_dist, action_diff = _get_k_edges(
                self.n_actions,
        ...
        assert (
            len(action_edges[0]) == N_ACTIONS * self.n_robots
        ), "Number of action edges is not num robots x n_actions"

        action_edges = (
            np.concatenate([action_edges[0], action_edges[1]], axis=0),
        ...
        if COMM_EDGES:
            ...
            comm_edges, comm_dist, comm_diff = _get_graph_edges(
                self.comm_radius, self.x[: self.n_robots, 0:2]
            )
            ...
            if not USE_POS_DELTA:
                ...
                edges_diff = np.concatenate(
                    (
                        np.concatenate((action_diff, comm_diff)).reshape((-1, 2)),
                        np.concatenate((action_dist, comm_dist)).reshape((-1, 1)),
                    ),
                    axis=1,
                )
        else:
            ...
            if not USE_POS_DELTA:
                ...
                edges_diff = np.concatenate(
                    (action_diff, action_dist.reshape((-1, 1))), axis=1
                )
        assert (
            len(senders) + self.n_motion_edges <= np.shape(self.senders)[0]
        ), "Increase MAX_EDGES"
        ...
        if LAST_EDGE_FEATURE:
            ...
            if self.last_loc is not None:
                for i in range(self.n_robots):
                    last_edges = np.logical_or(
                        last_edges,
                        np.logical_and(
                            receivers == i, senders == self.last_loc[i]
                        ).reshape((-1, 1)),
                    )
                    last_edges = last_edges.reshape((-1, 1))

            if not USE_POS_DELTA:
                edges = np.concatenate((last_edges, edges_dist), axis=1).reshape(
                    (-1, 2)
                )
            else:
                edges = np.concatenate((last_edges, edges_diff), axis=1).reshape(
                    (-1, 4)
                )
        ...
        if self.hide_nodes:
            seen_nodes = _nodes_within_radius(
                4.0 * DELTA,
                self.x[: self.n_robots, 0:2],
                self.x[0 : self.n_agents, 0:2],
            )
            self.discovered_nodes[0 : self.n_agents] = (
                self.discovered_nodes[0 : self.n_agents].reshape((-1, 1))
                + seen_nodes.astype(np.float)
            ) > 0.0
            self.nodes = self.nodes * self.discovered_nodes.reshape((-1, 1))

            frontier_node_ind = self.receivers[
                (1.0 - self.discovered_nodes[self.senders].flatten())
                * self.discovered_nodes[self.receivers].flatten()
                > 0.0
            ]
            self.nodes[frontier_node_ind, 3] = 1.0

            seen_edges = (
                self.discovered_nodes[self.senders].flatten()
                * self.discovered_nodes[self.receivers].flatten()
            )
        ...
        self.step_counter += 1
            ...
            self.step_counter == self.episode_length
            or np.sum(self.visited[self.n_robots :]) == self.n_targets
        )
    ...
    def reset(self):
        ...
        if graph_changed:
            ...
            if self.nearby_starts:
                n_nearest = self.get_n_nearest(
                    self.np_random.choice(self.n_targets),
                    self.n_robots * NEARBY_DENSITY,
                )
        ...
        nearest_landmarks = self.np_random.choice(
            np.arange(self.n_targets)[self.start_region],
            size=(self.n_robots,),
            replace=False,
        )
        ...
        unvisited_targets = (
            np.arange(self.n_targets)[self.unvisited_region] + self.n_robots
        )

        random_unvisited_targets = self.np_random.choice(
            unvisited_targets,
        ...
        if self.hide_nodes:
            self.discovered_nodes = np.vstack(
                (
                    np.ones((self.n_robots, 1)),
                    np.zeros((self.max_nodes - self.n_robots, 1)),
                )
            )
    ...
    def closest_targets(self):
        r = np.linalg.norm(
            self.x[: self.n_robots, 0:2].reshape((self.n_robots, 1, 2))
            - self.x[self.n_robots :, 0:2].reshape((1, self.n_targets, 2)),
            axis=2,
        )
    ...
    def render(self, mode="human"):
        ...
        if self.fig is None:
            ...
            for (i, j) in zip(self.motion_edges[0], self.motion_edges[1]):
                self.ax.plot(
                    [self.x[i, 0], self.x[j, 0]], [self.x[i, 1], self.x[j, 1]], "b"
                )
            ...
            a = gca()
            a.set_xticklabels(a.get_xticks(), fontdict=font)
            a.set_yticklabels(a.get_yticks(), fontdict=font)
        ...
        if self.hide_nodes:
            unvisited = np.where(
                (
                    np.logical_and(
                        self.visited[self.n_robots :] == 0,
                        self.discovered_nodes[self.n_robots : self.n_agents] > 0,
                    )
                ).flatten()
            )
            visited = np.where(
        ...
        if self.hide_nodes:
            self.line5.set_xdata(
                self.x[: self.n_agents, 0][self.nodes[: self.n_agents, 3] > 0]
            )
            self.line5.set_ydata(
                self.x[: self.n_agents, 1][self.nodes[: self.n_agents, 3] > 0]
            )
        ...
        if self.graph_cost is not None and horizon > -1:
            ...
            neighborhood = np.where(
                (self.graph_cost[robot_ind, :] <= horizon).flatten()
            )
    ...
    def _generate_targets(self):
        lattice = generate_lattice(
            (self.x_min, self.x_max, self.y_min, self.y_max), self.lattice_vectors
        )
        n_cities = 12
        # intercity_radius = self.x_max/6
        roads = generate_geometric_roads(n_cities, self.x_max, self.motion_radius)
        flag = np.min(
            np.linalg.norm(_get_pos_diff(lattice, roads), axis=2), axis=1
        ) <= (self.motion_radius / 1.4)
        targets = lattice[flag, :]
        r = np.linalg.norm(_get_pos_diff(targets), axis=2)
        r[r > self.motion_radius] = 0
        _, labels = connected_components(
            csgraph=csr_matrix(r), directed=False, return_labels=True
        )
    ...
    def _initialize_graph(self, targets):
        ...
        self.agent_type = np.vstack(
            (np.ones((self.n_robots, 1)), np.zeros((self.n_targets, 1)))
        )

        self.robot_flag = np.vstack(
            (np.ones((self.n_robots, 1)), np.zeros((self.n_targets, 1)))
        )

        self.landmark_flag = np.vstack(
            (np.zeros((self.n_robots, 1)), np.ones((self.n_targets, 1)))
        )
        ...
        self.motion_edges, self.motion_dist, self.motion_diff = _get_graph_edges(
            self.motion_radius, self.x[self.n_robots :, 0:2], self_loops=True
        )
        # cache motion edges
        self.motion_edges = (
            self.motion_edges[0] + self.n_robots,
            self.motion_edges[1] + self.n_robots,
        )
        ...
        if self.nearby_starts:
            n_nearest = self.get_n_nearest(
                self.np_random.choice(self.n_targets), self.n_robots * NEARBY_DENSITY
            )
            # n_nearest = self.get_n_nearest(self.np_random.choice(self.n_targets), 200)
            self.start_region = [i in n_nearest for i in range(self.n_targets)]
        else:
        ...
        if self.pad_nodes:
            nodes_space = Box(
                shape=(self.max_nodes, self.n_node_feat),
                low=-np.Inf,
                high=np.Inf,
                dtype=np.float32,
            )
            ...
            nodes_space = Box(
                shape=(self.n_agents, self.n_node_feat),
                low=-np.Inf,
                high=np.Inf,
                dtype=np.float32,
            )
        ...
        self.observation_space = gym.spaces.Dict(
            [
                ...
                (
                    "edges",
                    Box(
                        shape=(self.max_edges, N_EDGE_FEAT),
                        low=-np.Inf,
                        high=np.Inf,
                        dtype=np.float32,
                    ),
                ...
                (
                    "step",
                    Box(shape=(1, 1), low=0, high=EPISODE_LENGTH, dtype=np.float32),
                ),
            ]
        )
    ...
    def construct_time_matrix(self, edge_time=1.0):
        ...
        while changed_last_iter and np.sum(time_matrix) == np.Inf:
            ...
            for (sender, receiver) in zip(edges[0], edges[1]):
                new_cost = np.minimum(
                    time_matrix[:, sender] + edge_time, time_matrix[:, receiver]
                )

                prev[:, receiver] = np.where(
                    time_matrix[:, sender] + edge_time < time_matrix[:, receiver],
                    sender,
                    prev[:, receiver],
                )

                changed_last_iter = changed_last_iter or (
                    not np.array_equal(new_cost, time_matrix[:, receiver])
                )
    ...
    def get_number_nodes(ob_space, n_node_feat=None):
        if n_node_feat is None:
            n_node_feat = N_NODE_FEAT
        n_nodes = (ob_space.shape[0] - N_GLOB_FEAT) // (
            MAX_EDGES * (2 + N_EDGE_FEAT) + n_node_feat
        )
        return n_nodes
    ...
    def unpack_obs(obs, ob_space, dim_nodes=None):
        assert (
            tf is not None
        ), "Function unpack_obs() is not available if Tensorflow is not imported."
        ...
        n_nodes = (ob_space.shape[0] - N_GLOB_FEAT) // (
            MAX_EDGES * (2 + N_EDGE_FEAT) + dim_nodes
        )
        max_edges = MAX_EDGES
        ...
        cum_n_nodes = tf.cast(
            tf.reshape(tf.math.cumsum(n_node, exclusive=True), (-1, 1, 1)),
            dtype=tf.float32,
        )
        ...
        mask = tf.reshape(
            tf.not_equal(senders, -1), (batch_size, -1)
        )  # padded edges have sender = -1
    ...
    def controller(self, random=False, greedy=GREEDY_CONTROLLER, reset_solution=False):
        ...
        if self.hide_nodes:
            r[
                :,
                np.where(
                    np.equal(self.discovered_nodes[self.n_robots : self.n_agents], 0.0)
                ),
            ] = MAX_COST
        ...
        if greedy:
            ...
            assert (
                ortools is not None
            ), "Vehicle routing controller is not available if OR-Tools is not imported."

            if self.cached_solution is None or self.horizon > -1 or reset_solution:
                if self.horizon > -1:
                    self.cached_solution = solve_vrp(
                        self, min(self.horizon, EPISODE_LENGTH - self.step_counter)
                    )
                else:
        ...
        for i in range(self.n_robots):
            ...
            if (
                next_loc[i] == -1
                or self.graph_previous[
                    next_loc[i] - self.n_robots, curr_loc[i] - self.n_robots
                ]
                == -1
            ):
                ...
                u_ind[i] = np.where(
                    self.mov_edges[1][np.where(self.mov_edges[0] == i)] == next_step
                )[0][0]
